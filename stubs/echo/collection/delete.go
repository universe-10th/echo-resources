...
// DeleteEndpointStub implements a stub that provides an echo endpoint based
// on an implementation engine that deletes non-deleted elements.
type DeleteEndpointStub[IDT comparable, RT types.Resource[IDT]] struct {
	engine DeleteEndpointEngine[IDT, RT]
}

// NewDeleteEndpointStub creates a stub for deleting non-deleted collection elements.
...
func (deleteEndpointStub DeleteEndpointStub[IDT, RT]) Delete(context echo.Context) error {
	...
	_, element, err := retrieveScopedElement(engine, context, false)
	if err != nil {
		return err
	}
